sched_branch <- function(sched_name,trj_step,next_trj_step,continue){
  ...
  for (i in 1:(length(next_trj_step) - 1)) {
    ...
    \t\t\t",robs_log(trj_step,paste0('Entering branch trajectory ',i),ret=FALSE,pipe=TRUE),"
    \t\t\tset_attribute('item_next_block',",trj_step+ next_trj_step[i],"),
    ")  }
  subtrj_txt <- paste0(subtrj_txt,
"\t\ttrajectory('",sched_name,"_trajectory_",length(next_trj_step),"') %>% 
\t\t\t",robs_log(trj_step,paste0('Entering branch trajectory ',length(next_trj_step)),ret=FALSE,pipe=TRUE),"
 \t\t\tset_attribute('item_next_block',", trj_step+next_trj_step[length(next_trj_step)],")
\t)")
  
  
  trj_txt <- paste0("
             ",robs_log(trj_step,'Entering branch decision',ret=FALSE,pipe=TRUE),"
             ",robs_log(trj_step,'enter schedule queue',ret=FALSE,pipe=TRUE),"
             seize('",paste0("sched_res_block_",as.character(trj_step)),"',1) %>% 
             timeout(",tiny,") %>% 
             ",robs_log(trj_step,'finished tiny delay',ret=FALSE,pipe=TRUE),"
           branch( option = function() scheduler(sched_name='",sched_name,"',task='item:activity:',taskid=get_attribute(env,'item_id')),
                    ",continuetext,"\n",subtrj_txt," %>% 
                    release('",paste0("sched_res_block_",as.character(trj_step)),"',1)"
...
add_schedule_branch <- function(modelname,
  ...
  code <- sched_branch(sched_name,trj_step,next_trj_step,continue=continue)
  trj_txt <-
    paste0(
      start_code(trj_step = trj_step, next_trj_step = next_trj_step[1]),
      code,
      end_code(trj_step = trj_step)
    )
  env_txt <- paste0("env <-  env  %>%
  add_resource('",paste0("sched_res_block_",as.character(trj_step)),"', 1,preemptive = TRUE,preempt_order = 'fifo')")
  var_txt <- ""
